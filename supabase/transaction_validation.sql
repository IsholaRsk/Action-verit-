-- TRANSACTION VALIDATION FOR ADMIN - EscortHub
-- À exécuter dans Supabase SQL Editor
-- Ajoute validation admin avec anti double crédit, preuves, audit
...
-- deposit_requests avec preuve obligatoire
...
begin
  perform public.create_notification(NEW.user_id,'deposit_pending','Recharge en attente', format('Votre demande de %s %s est en cours de vérification. Réf: %s', NEW.amount, NEW.currency, NEW.transaction_reference), NEW.amount, NEW.currency, NEW.id, null, jsonb_build_object('payment_method',NEW.payment_method,'reference',NEW.transaction_reference));
  return NEW;
end;
$$ language plpgsql security definer;
...
begin
  ...
  insert into public.admin_logs(admin_id,action,target_table,target_id,details) values (auth.uid(),'approve_deposit','deposit_requests',req.id, jsonb_build_object('amount',req.amount,'user_id',req.user_id,'old_balance',old_bal,'new_balance',new_bal));
  perform public.create_notification(req.user_id,'deposit_approved','Recharge approuvée', format('Votre recharge %s %s approuvée. Nouveau solde %s %s. Réf: %s', req.amount, req.currency, new_bal, req.currency, req.transaction_reference), req.amount, req.currency, req.id, null, jsonb_build_object('old_balance',old_bal,'new_balance',new_bal,'transaction_id',trans_id));
  return json_build_object('success',true,'deposit_id',req.id,'amount',req.amount,'old_balance',old_bal,'new_balance',new_bal,'transaction_id',trans_id);
end;
$$ language plpgsql security definer;
...
begin
  if not public.is_admin() then raise exception 'Accès administrateur requis'; end if;
  if reason is null or char_length(trim(reason))<3 then raise exception 'Motif requis min 3 caractères'; end if;
  select * into req from public.deposit_requests where id=request_id for update;
  if not found then raise exception 'Demande introuvable'; end if;
  if req.status != 'pending' then raise exception 'Demande déjà traitée: %', req.status; end if;
  ...
  insert into public.admin_logs(admin_id,action,target_table,target_id,details) values (auth.uid(),'reject_deposit','deposit_requests',request_id, jsonb_build_object('amount',req.amount,'reason',trim(reason)));
  perform public.create_notification(req.user_id,'deposit_rejected','Recharge refusée', format('Votre recharge %s %s refusée. Motif: %s. Réf: %s', req.amount, req.currency, trim(reason), req.transaction_reference), req.amount, req.currency, req.id, null, jsonb_build_object('reason',trim(reason)));
...
begin
  ...
  if old_bal < prod.prix then
    ...
    raise exception 'Solde insuffisant: %s € < %s €. Veuillez recharger.', old_bal, prod.prix;
  end if;
...
select * from (values
  ('Crypto USDT TRC20', 'crypto', 'Envoyez USDT TRC20 à l''adresse. Montant exact. Preuve: hash TX.', 'TQn9Y2khEsLJW1ChVWFMSMeRDow5KcbLSE', 'TRC20', 'USDT', true),
  ('Transcash', 'transcash', 'Achetez carte Transcash. Photo claire ticket + code. Admin vérifie solde.', null, 'EUR', 'EUR', true),
  ('Recharge PCS', 'recharge', 'Achetez recharge PCS. Code + photo ticket.', null, 'PCS', 'EUR', true),
  ('Mobile Money', 'mobile_money', 'Envoyez Mobile Money Orange/MTN au +229 XX. Preuve: capture + ID.', null, 'MTN', 'EUR', true)
